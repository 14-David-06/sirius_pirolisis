...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (body.estadoRegistro) fields['Estado del Registro'] = body.estadoRegistro;
    ...
    const res = await fetch(url, {
      ...
      // typecast permite crear opciones de singleSelect (p. ej. Autorizado / No autorizado)
      body: JSON.stringify({ fields, typecast: true }),
    });

    if (!res.ok) {
      const err = await res.text();
      console.error('[nomina/novedades PATCH] Airtable error:', err);
    ...
    console.error('[nomina/novedades PATCH]', e);
