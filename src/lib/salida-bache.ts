...
//   Sirius Inventario Production Core — una `Salida` de `Biochar Puro` con
//     `bache_origen_id` y `produccion_destino_id` = la referencia de esta salida. Es
//     el libro mayor: lo que dice es lo que se puede despachar. (Hasta el
//     2026-08-21 esa Salida iba a Insumos Core; ver `biochar-inventario-core.ts`.)
...
export interface SalidaBacheInput {
   ...
   *
   * Es lo que queda en `responsable` del movimiento del Core. Inventario
   * Production Core no tiene campo de SIRIUS-PER —a diferencia de Insumos Core, que
   * sí lo tenía—, así que el nombre es toda la atribución posible ahí; el
   * SIRIUS-PER sigue guardándose en los documentos de PiroliApp que lo tienen.
   */
...
async function buscarSalidaExistente(
  ...
  // `contarMovimientosPorDestino` lanza si la consulta falla: ante la duda NO se
  // asume que ya existe.
  const movimientosCore = await contarMovimientosPorDestino(referencia);
...
/**
 * Salida de `Biochar Puro` en Sirius Inventario Production Core: el libro mayor.
 *
 * Sin este paso el saldo del producto queda por encima del real y, como el Core
 * manda en `resolverBiocharDisponible()`, la app seguiría ofreciendo para producir
 * un biochar que ya salió de la planta.
 *
 * La `referencia` va en `produccion_destino_id` —el mismo campo donde una
 * producción escribe su lote `BLEND-…`— y también en `documento_referencia`: es la
 * llave por la que `buscarSalidaExistente` reconoce que esta salida ya se registró.
 */
async function escribirSalidaCore(
  bache: BacheResuelto,
  kg: number,
  referencia: string,
  input: { motivo: MotivoSalida; destino: string; fecha: string; realizaRegistro: string }
): Promise<StepResult> {
  if (!credencialesBiocharPuro()) {
    return {
      ...
      error:
        ...
        'AIRTABLE_INVENTARIO_BIOCHAR_PURO_PRODUCT_ID): la salida no se registró en el libro ' +
  ...
  try {
    const { movimientoId, kg: cantidad, vinculadoAlStock } = await crearMovimientoBiocharPuro({
      ...
      produccionDestino: referencia,
      documentoReferencia: referencia,
      motivo: `Salida de biochar — ${etiqueta}`,
      fecha: input.fecha,
      responsable: input.realizaRegistro,
      ubicacionDestino: input.destino,
      observaciones:
        `Salida de biochar — ${etiqueta} · destino ${input.destino} · bache ${bache.codigo} ` +
        marcaSalida(referencia),
    });
    ...
    return { step: 'biochar_core', ok: true, detail: { movimientoId, kg: cantidad } };
  } catch (err) {
    return {
      step: 'biochar_core',
      ok: false,
        ...
        },
        {
          ...
          detail:
            existente.movimientosCore > 0
              ...
              : { porEscribir: { tipo: 'Salida', kg: Number(kg.toFixed(2)), produccionDestino: referencia } },
        },
        {
          step: 'estado_bache',
          ok: true,
