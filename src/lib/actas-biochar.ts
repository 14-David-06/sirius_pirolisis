...
//
// ═══ DOS DESCUENTOS, NO UNO ═══════════════════════════════════════════════════
// El "tipo de biochar" del acta no es cosmético: decide de dónde se descuenta.
// Desde el 2026-08-21 los dos son productos de Sirius Inventario Production Core
// —el biochar puro dejó de ser un insumo—, pero se descuentan por caminos distintos:
//
//   Biochar Puro  → trazabilidad BACHE POR BACHE (es lo que sostiene la contabilidad
//                   de carbono). Se reusa `runSalidaBache()` una vez por bache, que
//                   escribe el detalle en PiroliApp, la Salida del producto y el
//                   `Estado Bache`.
//   Biochar Blend → sale de un LOTE `BLEND-…` ya producido, con una sola Salida
//                   cuyo `documento_referencia` es `ACTA-<código>`.
//
// ═══ TODO EN MASA SECA ════════════════════════════════════════════════════════
// Los KG del acta son SIEMPRE masa seca (decisión de David, 2026-08-05), igual que
// el inventario (`Total Cantidad Actual Biochar Seco` y el producto del Core). No hay
